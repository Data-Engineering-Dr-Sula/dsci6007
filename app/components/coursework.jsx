...
export default function CourseWork() {
  ...
  const getFoldersUI = (folder) => {
    if (folder.isFolder) {
      return (
        <Accordion
          ...
          type="single"
          collapsible
          className="w-full bg-gray-50 px-4"
        >
          <AccordionItem
            value={folder.path}
            onClick={() => getFiles(folder.path)}
          >
            <AccordionTrigger>📂 {folder.name}</AccordionTrigger>
            <AccordionContent>
              {subItems[folder.path]?.length > 0
                ? subItems[folder.path]?.map((subItems) =>
    ...
    return (
      <Accordion key={folder.name} className="w-full bg-gray-50 px-4">
